...
| 2   | **一天两期：`news-am` 09:10 / `news-pm` 18:10**     | 避开 `:00` `:30`（全球 Actions 最拥挤的两分钟），同早晚报的既有理由                |
...
| --------- | ------- | --------------: | --------------------------------------------------------------- |
| `news-am` | `09:10` |              24 | 24h 保证「晚间那期跳过」时早间仍能兜住，同 `morning` 的既有理由 |
| `news-pm` | `18:10` |              12 | 覆盖 09:10 之后的新增，余量吸收 Actions 0–30 分钟排队漂移       |
...
```
10 1 * * *    news-am - 09:10 Asia/Shanghai
10 10 * * *   news-pm - 18:10 Asia/Shanghai
```
