...
| 2   | **一天两期：`news-am` 06:40 / `news-pm` 19:10**     | 避开 `:00` `:30`（全球 Actions 最拥挤的两分钟），同早晚报的既有理由                |
...
| --------- | ------- | --------------: | --------------------------------------------------------------- |
| `news-am` | `06:40` |              24 | 24h 保证「晚间那期跳过」时早间仍能兜住，同 `morning` 的既有理由 |
| `news-pm` | `19:10` |              13 | 上溯到 06:10，盖住 06:40 那期并留 30 分钟吸收排队漂移           |
...
```
40 22 * * *   news-am - 06:40 Asia/Shanghai（前一 UTC 日）
