...
| 期次    | CST   | UTC 触发 | 峰 / 谷          |
| ------- | ----- | -------- | ---------------- |
| morning | 07:10 | 23:10    | **谷时（半价）** |
| news-am | 07:40 | 23:40    | **谷时（半价）** |
| evening | 20:10 | 12:10    | **谷时（半价）** |
| news-pm | 19:10 | 11:10    | **谷时（半价）** |
| weekly  | 08:00 | 00:00    | **谷时（半价）** |

**五期全部落在谷时，输入输出一律半价。** §4 的 ¥3.51 是按更保守的口径算的，实际更低。

> **2026-08-26 调整**：news-am 原为 09:10 CST = UTC 01:10，是唯一踩在峰时里的一期；
> 一并把 news-pm 从 18:10 挪到 19:10。挪 news-am 省下的钱其实微不足道
> （那是最便宜的一期：`oneline` 风格、不抓正文、只喂 ≈100 字符 excerpt，
> 每月约 ¥0.4），真正的动因是作息；半价只是顺手白捡的。

**改 cron 时间会悄悄破坏这件事**。换算规则：峰时按北京时间是
**工作日 09:00–12:00 和 14:00–18:00**，把新时间往这两个区间里比一下即可；
周六周日无论几点都是谷时。注意 Actions 的排队漂移可达 0–30 分钟，
贴着区间边界排（比如 08:50 或 18:05）等于把自己交给运气。
