 ...
 */
import { composeAlert } from './alert/compose'
...
async function postMail(subject: string, content: string): Promise<void> {
...
async function main(): Promise<void> {
  ...
  const { subject, content } = composeAlert(env)
  ...
  const results = await Promise.allSettled([
    ...
    postMail(subject, content.replace(/\*\*/g, '')),
  ])
